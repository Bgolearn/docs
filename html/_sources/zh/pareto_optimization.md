...
```{note}
本页涵盖多目标材料设计的帕累托前沿分析、权衡理解和决策策略。
```

## 了解帕累托前沿

帕累托前沿代表了竞争目标之间的一组最佳权衡。在材料设计中，这有助于我们了解材料特性之间的基本限制和关系。
...
每个点都代表一个最佳的权衡——你不能在不牺牲另一个属性的情况下改进一个属性。
```
...
运行 MultiBgolearn 优化后，分析结果以确定 Pareto 最优解：
...
2. **尝试真实材质案例**：{doc}`examples/materials_design`
3. **学习先进技术**：{doc}`examples/advanced_usage`
4. **探索可视化工具**：{doc}`visualization`

```{seealso}
有关多目标优化的更多信息：
- Deb, K.“使用进化算法的多目标优化”
- Miettinen, K.“非线性多目标优化”
- Zitzler, E.“多目标进化算法：比较案例研究”
```
