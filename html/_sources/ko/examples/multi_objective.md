...
```{note}
이 페이지는 Bgolearn 매뉴얼의 한국어 버전입니다.
```
...
<!-- ```{important}
**MultiBgolearn Limitation**: The current version of MultiBgolearn **only supports bi-objective optimization** (`object_num=2`).
All examples in this page use exactly 2 objectives. For more than 2 objectives, please use the weighted sum approach
shown in the single-objective examples with Bgolearn.
``` -->
...
### Multi-Objective Optimization with EHVI
...
### Compare Different MOBO Algorithms
...
## Example 2: Bi-Objective Processing Optimization
...
## Example 3: Materials Discovery with Constraints
...
### Common Error: TypeError with GaussianProcess
...
**Cause**: This is a bug in older versions of MultiBgolearn.
...
Or use a different surrogate model:
```python
VS_recommended, improvements, index = bgo.fit(
    'dataset.csv',
    'virtual_space.csv',
...
- **Learn Pareto analysis**: {doc}`../pareto_optimization`
- **Understand MOBO theory**: {doc}`../multi_objective_concepts`
